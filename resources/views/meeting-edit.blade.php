            ...
            <div class="card mb-3">
                ...
                <div class="card-body">
                    ...
                    <div class='form-row'>
                        <div class="form-group col">
                            <label>Status *</label>
                            {!! Form::_select('status', $select->meet_status??[], $data->status??'' ) !!} 
                        </div>
                        <div class="form-group col">
                            <label>Location</label>
                            <input name="location" type="text" class="form-control form-control-sm" value="{{$data->location??''}}">
                        </div>
                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $("button#cmSave").click(async function(e){ //using ajax
                ...
                var id = '{{$id}}';
                var resp = await axios.post("{{env('API_URL')}}/api/meeting/save/"+id, formdata);
                //console.log(resp)
                if (resp.status==200) {
